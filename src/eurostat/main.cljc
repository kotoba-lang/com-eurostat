(ns eurostat.main
  "Kotodama WASM entrypoint for the Eurostat clean-room actor (L4) — Clojure port.
  ...
  facts (`eurostat.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "eurostat")
(def tier "L4")
...
(def entity-specs
  [{:entity "Filing"      :plural "filings"      :id-prefix "eurostat_fil"
    :fields [:filerId :scheme :period :status]     :required [:scheme :period]
    :coerce {}                       :refs {}}
   {:entity "Citizen"     :plural "citizens"     :id-prefix "eurostat_cit"
    :fields [:nationalId :jurisdiction]             :required [:jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Service"     :plural "services"     :id-prefix "eurostat_ser"
    :fields [:name :department]                     :required [:name :department]
    :coerce {}                       :refs {}}
   {:entity "Application" :plural "applications" :id-prefix "eurostat_app"
    :fields [:citizenId :serviceId :status]         :required [:status]
    :coerce {}                       :refs {:citizenId "Citizen" :serviceId "Service"}}
   {:entity "Dataset"     :plural "datasets"     :id-prefix "eurostat_dat"
    :fields [:name :publisher :updatedAt]           :required [:name :publisher]
    :coerce {}                       :refs {}}
   {:entity "Identity"    :plural "identities"   :id-prefix "eurostat_ide"
...
(defn emit-facts
  "EAVT facts for one record: {\"eurostat.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "eurostat-compat" :tier tier :entities entities} 200])
...
(defn start! [] :eurostat-compat/ready)
